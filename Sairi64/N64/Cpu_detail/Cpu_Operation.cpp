...
namespace N64::Cpu_detail
{
	...
	public:
		...
		static OperatedUnit ADD(Cpu& cpu, InstructionR instr)
		{
			...
			N64_TRACE(U"ADD: {} <= {} + {}"_fmt(instr.RdName(), instr.RsName(), instr.RtName()));
		}
	};

	[[nodiscard]]
	OperatedUnit operateSpecial(N64System& n64, Cpu& cpu, InstructionR instr)
	{
		switch (instr.Funct())
		{
	...
	[[nodiscard]]
	OperatedUnit operateInstructionInternal(N64System& n64, Cpu& cpu, Instruction instr)
	{
		switch (instr.Op())
		{
		case Opcode::SPECIAL:
	...
	{
		return (void)operateInstructionInternal(n64, cpu, instr);
	}
}
